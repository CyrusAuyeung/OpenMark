...
- `OpenMark.Setup.0.21.1.exe`
- `OpenMark.0.21.1.exe`
- `OpenMark.Setup.0.21.1.exe.blockmap`

Expected macOS artifacts:

- `OpenMark-0.21.1.dmg`
- `OpenMark-0.21.1-arm64.dmg`
- `OpenMark-0.21.1-mac.zip`
- `OpenMark-0.21.1-arm64-mac.zip`

Expected Linux artifacts:

- `openmark-editor_0.21.1_amd64.deb`
...
- Enter a tag like `v0.21.1` to publish a GitHub release from the workflow run. The tag is validated as semantic version style and points at the workflow commit.
- Manual releases default to draft mode so artifacts and notes can be reviewed before publishing.
- Enable `prerelease` for preview builds such as `v0.21.1-beta.1`.
