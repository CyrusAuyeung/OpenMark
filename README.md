...
[![Release version](https://img.shields.io/badge/release-v0.10.0-2f7a5f)](https://github.com/CyrusAuyeung/OpenMark/releases/tag/v0.10.0)
...
| Platform | Package |
| --- | --- |
| Windows installer | `OpenMark.Setup.0.10.0.exe` |
| Windows portable app | `OpenMark.0.10.0.exe` |
| macOS Intel | `OpenMark-0.10.0.dmg` |
| macOS Apple Silicon | `OpenMark-0.10.0-arm64.dmg` |
| Linux Debian/Ubuntu | `openmark-editor_0.10.0_amd64.deb` |
...
- Find and replace with match case, whole word, result snippets, click-to-jump navigation, replace next, and replace all
- Preview heading jumps and split-view editor/preview scroll synchronization
...
- Preview the export document from the toolbar, command palette, or desktop File menu
- Copy Markdown or HTML from the toolbar, command palette, or desktop Edit menu
...
| 平台 | 安装包 |
| --- | --- |
| Windows 安装版 | `OpenMark.Setup.0.10.0.exe` |
| Windows 便携版 | `OpenMark.0.10.0.exe` |
| macOS Intel | `OpenMark-0.10.0.dmg` |
| macOS Apple Silicon | `OpenMark-0.10.0-arm64.dmg` |
| Linux Debian/Ubuntu | `openmark-editor_0.10.0_amd64.deb` |
...
- 查找替换支持大小写匹配、全词匹配、结果片段、点击跳转、替换当前项和全部替换
- 预览标题可跳回编辑器，分屏模式下编辑器和预览滚动同步
...
- `Ctrl+E`：导出 HTML
- `Ctrl+Shift+E`：导出 PDF
- 可通过工具栏、命令面板或桌面 File 菜单预览导出文档
- 可通过工具栏、命令面板或桌面 Edit 菜单复制 Markdown 或 HTML
- `Ctrl+F`：在当前文档中查找
- `Ctrl+H`：在当前文档中替换
- `Ctrl+P`：快速打开工作区和最近文件
- `Ctrl+Shift+P`：打开命令面板
